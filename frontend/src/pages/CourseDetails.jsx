...
import CourseAccordionBarTest from "../test/CourseAccordionBarTest.jsx";
...
function CourseDetails() {
    ...
    const handleBuyCourse = () => {
        if (token) {
            ...
            buyCourse(token, coursesId, user, navigate, dispatch).then(r => {
            })
            return
        }
        ...
        <>
            <div className={`relative w-full bg-richblack-800`}>
                ...
                <div className="mx-auto box-content px-4 lg:w-[1260px] 2xl:relative ">
                    <div
                        className="mx-auto grid min-h-[450px] max-w-maxContentTab justify-items-cente py-8 lg:mx-0 lg:justify-items-start lg:py-0 xl:max-w-[810px]">
                        ...
                        <div className={`mb-5 flex flex-col justify-center gap-4 py-5 text-lg text-richblack-5`}>
                            <span className="text-4xl font-bold text-richblack-5 sm:text-[42px]">{courseName}</span>
                            <span className='text-richblack-200'>{courseDescription}</span>
                            ...
                            <span className="capitalize "> Created By <span
                                className="font-semibold underline">{instructor.firstName} {instructor.lastName}</span></span>
                            <div className="flex flex-wrap gap-5 text-lg">
                <span className="flex items-center gap-2">
                  {" "}
                    <BiInfoCircle/> Created at {formatDate(createdAt)}
                </span>
                                <span className="flex items-center gap-2">{" "} <HiOutlineGlobeAlt/> English</span>
                            </div>
                        </div>

                        {/* will appear only for small size */}
                        <div className="flex w-full flex-col gap-4 border-y border-y-richblack-500 py-4 lg:hidden">
                            <p className="space-x-3 pb-4 text-3xl font-semibold text-richblack-5">{coinSymbol} {price}</p>
                            <button className="yellowButton" onClick={handleBuyCourse}>Add Course</button>
                        ...
                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                            {
                                tag && tag.map((item, ind) => (
                                    <span key={ind} className="bg-yellow-50 p-[2px] text-black rounded-full text-center font-semibold">
                    {item}
                  </span>
                                ))
                            }
