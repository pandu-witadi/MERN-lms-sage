...
function TemplateLogin({title}) {
    ...
    const handleOnSubmit = async (e) => {
        ...
        dispatch(await http_login(email, password)).then(response => {
            setLoading(false);
            if (response.status) {
                ...
                <div className="flex flex-col items-center justify-center">
                    ...
                    <div className="text-[1.6rem] md:text-[1.7rem] font-bold text-center mt-3">{t('byCreator')}</div>
                    {
                        apiResponse.status ?
                            ...
                            (<div
                                className="whitespace-pre-line text-[1.0rem] md:text-[1.2rem] font-semibold text-center mt-[30px] mb-[10px] text-error">
                                {apiResponse.msg}
                            </div>)
                    }
