...
const Navbar = () => {
  ...
  const createDashboardNav = (link) => {
      let path_ = link?.path;
      let title_ = link?.title;
      if((token !== null) && (path_ === '/')) {
          path_ = '/dashboard';
          title_ = 'Dashboard';
      }
      ...
      return(
        <>
            <Link to={path_}>
                <p className={`${matchRoute(path_) ? "bg-yellow-25 text-black" : "text-richblack-25"} rounded-xl p-1 px-3 `}>
                    {title_}
                </p>
            </Link>
        </>
              ...
              <li key={index}>
                {
                  link.title === "Catalog" ? (
                    ...
                    >
                      ...
                      >
                        <div
                          className="absolute left-[50%] top-0 z-[100] h-6 w-6 translate-x-[80%] translate-y-[-40%] rotate-45 select-none rounded bg-richblack-5"></div>
